...
At the time of the original audit, the repository contained two notebooks, a README, and seven PNGs. Some files were added later; see [the data inventory](data-inventory.md). The original audit did not contain `505_trend.csv`, `data_sentiment_merged_v3.csv`, `df_reviews.pkl`, the saved BERT checkpoint, the scraping/linkage/translation scripts, or dependency locks. The transformation connecting BERT review predictions to the four forecasting sentiment columns is absent. Actual data coverage, duplicates, class balance, date alignment, feature provenance, and metric reproducibility therefore remain unverified.
